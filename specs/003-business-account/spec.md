...
A bank staff member (or demo operator) visits `/business/create` and fills in the business name, UEN, address, and an initial deposit (minimum 7,000). On submit, the system runs mock SQL that creates: a business account with the deposited balance, an account manager user, and an authoriser user. The generated credentials for both users are displayed once on the confirmation screen. The initial deposit is recorded as a deposit transaction in the business account's transaction history.
...
**Independent Test**: Visit `/business/create`, fill in valid details with an initial deposit ≥ 7,000, submit, and confirm that credentials for both the account manager and authoriser are shown, both users can subsequently log in, and the initial deposit appears in the business account transaction history.

**Acceptance Scenarios**:

1. **Given** a visitor is on `/business/create`, **When** they submit a valid business name, UEN, address, and initial deposit ≥ 7,000, **Then** the system creates a business account (with that balance), an account manager user, and an authoriser user — and displays the generated credentials for both on a confirmation screen.
...
4. **Given** a visitor submits the creation form with an initial deposit below 7,000, **When** the form is submitted, **Then** a validation error is shown for the deposit field and no account is created.
5. **Given** a visitor submits a valid creation form with an initial deposit ≥ 7,000, **When** the account is created, **Then** the initial deposit is recorded as a deposit transaction in the business account's transaction history.

---

### User Story 2 - Account Manager Submits a Transaction (Priority: P1)

The account manager logs into their generated account and navigates to the business account view. They can submit any transaction type on behalf of the business account. Deposits execute immediately; outgoing transactions (withdrawal, transfer, bill payment) enter a pending state awaiting authoriser approval. Any outgoing transaction that would result in a business account balance below 7,000 is blocked immediately with an error.
...
3. **Given** an account manager submits an outgoing transaction where the resulting balance would be below 7,000, **When** the form is submitted, **Then** a clear error is shown and no pending transaction is created.

---

### User Story 3 - Authoriser Approves or Rejects a Pending Transaction (Priority: P1)

The authoriser logs into their generated account. If pending transactions exist for their business account, a visible link to the pending queue appears on their dashboard. They can approve or reject each transaction individually. If approving a transaction would bring the business account balance below 7,000, the transaction is automatically rejected and recorded as "Rejected".
...
5. **Given** the authoriser attempts to approve a pending transaction that would bring the business account balance below 7,000, **When** they approve, **Then** the transaction is automatically rejected and recorded as "Rejected" in the business account transaction history; the balance is unchanged.
...
- What happens when the initial deposit on the creation form is below 7,000? → Validation error on the deposit field; account not created.
- What happens if the account manager submits an outgoing transaction where the resulting balance would be below 7,000? → Error shown; no pending transaction created.
- What happens if the authoriser approves a transaction that would bring the balance below 7,000? → Transaction is automatically rejected and recorded as "Rejected" in transaction history; balance is unchanged.
- What is the minimum balance threshold? → ≥ 7,000 (inclusive); a resulting balance of exactly 7,000 is acceptable.
...
- **FR-001**: The system MUST provide a public page at `/business/create` (no login required) with a form accepting: business name, UEN, address (street, city, postal code), and an initial deposit amount.
...
- Q: Does the $7,000 minimum apply only at creation or must the account maintain it at all times? → A: Both — balance must be ≥ 7,000 upon creation and maintained at all times (supersedes Session 2026-05-24 answer).
- Q: How is the initial 7,000 minimum balance established at creation? → A: Creation form includes a required initial deposit field; minimum accepted value is 7,000.
- Q: At which point(s) is the minimum balance enforced for outgoing transactions? → A: Both at account manager submission and at authoriser approval.
- Q: What happens when an authoriser approves a transaction that would breach the 7,000 minimum? → A: Transaction is automatically rejected and recorded as "Rejected" in the business account transaction history.
- Q: Is the 7,000 minimum floor inclusive or exclusive? → A: Inclusive — a resulting balance of exactly 7,000 is acceptable (balance ≥ 7,000).
- Q: Does the initial deposit on the creation form appear in the business account transaction history? → A: Yes — recorded as a deposit transaction.
...
- Q: Does the $7,000 minimum apply only at registration or must the account maintain it at all times? → A: Both — minimum applies at creation and at all times (superseded and fully detailed in Session 2026-05-26).
